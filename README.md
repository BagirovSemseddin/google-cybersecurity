# Google Cybersecurity Certificate – Learning Journey

This repository documents my learning progress through the Google Cybersecurity Certificate program.

## Focus Areas
- Security fundamentals and risk management
- Defensive security and SOC mindset
- Industry frameworks (NIST CSF, OWASP, CIA Triad)
- Practical portfolio activities and reflections

## Structure
Each course includes:
- Module summaries
- Key concepts and reflections
- Portfolio-based security activities

## Disclaimer
This repository is for educational and documentation purposes only.
No offensive security tools, exploits, or real-world attack instructions are included.

## Skills Developed
- Security risk identification and prioritization
- Basic SOC workflow understanding
- Log analysis fundamentals
- Incident response lifecycle awareness

## How This Translates to Real-World SOC Work
This repository reflects hands-on learning aligned with entry-level SOC Analyst responsibilities such as monitoring alerts, understanding risk impact, and supporting incident response processes.

## Current Status
- Course 1: Foundations of Cybersecurity – completed
- Course 2: Play It Safe – Managing Security Risks - completed
- Course 3: Connect and Protect: Networks and Network Security - completed
- Course 4: Tools of the Trade: Linux and SQL ( in progress )
